...
function MeshGeneration(props: any) {
  const geo: MutableRefObject<PlaneGeometry> | MutableRefObject<null> =
    useRef(null);
  useEffect(() => {
    if (geo.current === null) {
      console.log("ERROR geo.current = null");
      return;
    }
    const PLANE = geo as MutableRefObject<PlaneGeometry>;
    ...
    for (let x = 0; x < celllsAcross_x; x++) {
      for (let y = 0; y < cellssDeep_y; y++) {
        const element = (x * img.width + y) * 4;
        const height = data[element] / 32;
        console.log(x, y, element, height);
        const count = PLANE.current.attributes.position.count;
        for (let index = 0; index < count; index++) {
          const x = PLANE.current.attributes.position.getZ(index);
          const dx = x + height;
          PLANE.current.attributes.position.setX(index, dx);
        }
      }
    }
    img.remove();
